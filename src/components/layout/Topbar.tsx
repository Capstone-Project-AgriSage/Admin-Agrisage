...
export default function Topbar({ onMenuClick }: TopbarProps) {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    const accountMatch = accountsService.list().find(
      (a) => a.id.toLowerCase().includes(q) || a.fullName.toLowerCase().includes(q) || a.phone.includes(q),
    )

    if (accountMatch) {
      navigate('/accounts')
      showToast(`Tìm thấy tài khoản: ${accountMatch.fullName} (#${accountMatch.id})`)
    } else {
      ...
      <div className="flex items-center justify-end gap-space-md shrink-0">
        <form className="relative hidden lg:block w-72" onSubmit={handleSearchSubmit}>
          ...
          <input
            ...
            placeholder="Tìm tài khoản theo tên, mã, SĐT..."
            type="text"
            value={searchQuery}
